...
#' 
#' @return \describe{
#' \item{lmforwardsequentialAICc}{An \code{\link{lm}-class} object.}
#' \item{lmforwardsequentialsidak}{An \code{\link{lm}-class} object.}
...
lmforwardsequentialAICc <- function (y, x, object) {
  ...
  if(!inherits(object,"PEM"))
    stop("Argument 'object' must be of class 'PEM'")
    ...
    paste(
      if(length(included)) {
        paste(
          if(AuxTrait) "+",
          paste(
            paste("V_", included, sep = ""),
            collapse = "+"
          )
        )
      } else if(AuxTrait) "" else "1",
      sep=""
    ) -> p2
...
lmforwardsequentialsidak <- function (y, x, object, alpha = 0.05) {
  ...
  while (TRUE) {
    ...
    if (min(pval, na.rm = TRUE) < alpha) {
      ...
      pow <- ncol(object$u):(ncol(object$u) - length(included) + 1L)
